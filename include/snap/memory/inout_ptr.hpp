...
namespace snap {
    ...
    class inout_ptr_t {
        static_assert(!is_specialization_of_v<Smart, std::shared_ptr>,
			"Using std::shared_ptr<> without a deleter in snap::inout_ptr is not supported.");
    ...
    public:
        ...
        {
            if constexpr (is_releasable_smart_pointer_v<Smart>) {
                m_smart.release();
            } else {
                m_smart = Smart();
            }
    ...
    ~inout_ptr_t() {
        ...
        if constexpr (std::is_pointer_v<Smart>) {
            ...
            std::apply([&](auto&&... a) {
                m_smart.reset(static_cast<SmartPtr>(m_ptr),
            ...
            std::apply([&](auto&&... a) {
                m_smart = Smart(static_cast<SmartPtr>(m_ptr),
                                std::forward<decltype(a)>(a)...);
            }, std::move(m_args));
        }
    }
    ...
    operator Pointer*() const noexcept {
        return std::addressof(const_cast<Pointer&>(m_ptr));
    }
