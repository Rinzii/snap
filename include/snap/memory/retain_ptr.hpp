...
struct retain_ptr
{
    ...
    SNAP_CONSTEXPR20 retain_ptr(pointer ptr, retain_object_t)
    ...
    SNAP_CONSTEXPR20 retain_ptr(pointer ptr, adopt_object_t)
        : ptr { ptr }
    {
    }

    explicit SNAP_CONSTEXPR20 retain_ptr(pointer ptr)
        : retain_ptr { ptr, default_action() }
    {
    }

    SNAP_CONSTEXPR20 retain_ptr(nullptr_t)
        : retain_ptr { }
    {
    }

    SNAP_CONSTEXPR20 retain_ptr(retain_ptr const &that)
        : ptr { that.ptr }
    {
        if (*this)
        {
    ...
    SNAP_CONSTEXPR20 retain_ptr(retain_ptr &&that) noexcept
        : ptr { that.detach() }
    {
    }
    ...
    SNAP_CONSTEXPR20 void reset(pointer ptr, retain_object_t)
    {
        *this = retain_ptr(ptr, retain_object);
    }

    SNAP_CONSTEXPR20 void reset(pointer ptr, adopt_object_t) noexcept
    {
        *this = retain_ptr(ptr, adopt_object);
    }

    SNAP_CONSTEXPR20 void reset(pointer ptr)
    {
...
SNAP_CONSTEXPR20 retain_ptr<T> make_retain(Args &&... args)
{
...
constexpr retain_ptr<To, R> static_retain_pointer_cast(const retain_ptr<From, R> &p) noexcept
{
    return retain_ptr<To, R>(static_cast<To *>(p.get()));
}

template<class To, class From, class R>
constexpr retain_ptr<To, R> dynamic_retain_pointer_cast(const retain_ptr<From, R> &p) noexcept
{
...
struct std::hash<snap::retain_ptr<T, R>>
{
    std::size_t operator()(const snap::retain_ptr<T, R> &p) const noexcept
    {
        return std::hash<typename snap::retain_ptr<T, R>::pointer>()(p.get());
    }
};
...
template<class T, class R>
constexpr void swap(retain_ptr<T, R> &lhs, retain_ptr<T, R> &rhs) noexcept
{
    lhs.swap(rhs);
}

template<class T, class R>
constexpr bool operator==(retain_ptr<T, R> const &lhs, retain_ptr<T, R> const &rhs) noexcept
{
...
template<class T, class R>
constexpr bool operator==(retain_ptr<T, R> const &lhs, nullptr_t) noexcept
{
    return !lhs;
}
...
template<class T, class R>
constexpr bool operator==(nullptr_t, retain_ptr<T, R> const &rhs) noexcept
{
    return !rhs;
}
