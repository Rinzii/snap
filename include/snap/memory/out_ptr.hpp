...
namespace snap {
    ...
    public:
        ...
        explicit out_ptr_t(Smart& smart, Args... args) noexcept
			: m_smart(smart), m_args(std::forward<Args>(args)...), m_ptr()
		{
			using Ptr = decltype(smart);
            if constexpr (is_resettable_smart_pointer_v<Ptr>) {
                m_smart.reset();
            } else if constexpr (std::is_constructible_v<Smart>) {
                m_smart = Smart();
            } else {
        ...
        ~out_ptr_t() {
            ...
            if constexpr(is_resettable_smart_pointer_with_args_v<Smart, Pointer, Args...>) {
                std::apply([&](auto&&... m_args) {
                        m_smart.reset(static_cast<SmartPtr>(m_ptr), std::forward<Args>(m_args)...);
                    },
                    std::move(m_args));
            } else {
                ...
                std::apply([&](auto&&... m_args) {
                        m_smart = Smart(static_cast<SmartPtr>(m_ptr), std::forward<Args>(m_args)...);
                    },
                    std::move(m_args)
                );
            }
        ...
        template<class P = Pointer, std::enable_if_t<!std::is_same_v<P, void*>, int> = 0>
        operator void**() const noexcept
        {
            static_assert(std::is_pointer_v<Pointer>,
                        "Conversion to void** requires Pointer to be a raw pointer.");
