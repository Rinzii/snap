...
namespace snap {
    ...
    template <typename T>
    [[nodiscard]] constexpr std::enable_if_t<std::is_unsigned_v<T>, int>
    popcount(T value)
    {
    ...
    constexpr int popcount(unsigned int n)
    {
        ...
        if constexpr (std::numeric_limits<unsigned int>::digits == 32) { // 32-bit int
            n = n - ((n >> 1) & 0x55555555u);
    ...
    constexpr int popcount(unsigned long n)
    {
        ...
        if constexpr (std::numeric_limits<unsigned long>::digits == 32) { // 32-bit long
