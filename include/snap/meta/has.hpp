...
namespace snap {
    ...
    inline constexpr bool has = ::snap::is_detected_v<Expr, Ts...>;

    template<template<class...> class Expr, class To, class... Ts>
    inline constexpr bool has_as = ::snap::is_detected_convertible_v<To, Expr, Ts...>;

    template<template<class...> class Expr, class T, class U>
    inline constexpr bool has_either = has<Expr, T, U> || has<Expr, U, T>;

    template<template<class...> class Expr, class To, class T, class U>
    inline constexpr bool has_either_as = has_as<Expr, To, T, U> || has_as<Expr, To, U, T>;
    ...
    namespace op {
        ...
        namespace details {
            ...
            template<class C, class M> struct member_pointer_class<M C::*> { using type = C; };

            template<class PM>
            using member_pointer_class_t = typename member_pointer_class<PM>::type;

            template<class T>
            using decay_t = std::remove_cv_t<std::remove_reference_t<T>>;

            template<class T>
            using pointee_t = std::remove_cv_t<std::remove_pointer_t<decay_t<T>>>;
        }

        // object .* pointer-to-member (object on LHS)
        template<class T, class PM>
        using member_pointer_access =
            std::enable_if_t<
                std::is_member_pointer_v<PM> &&
                std::is_base_of_v<details::member_pointer_class_t<PM>, details::decay_t<T>>,
                int>;

        // pointer ->* pointer-to-member (pointer on LHS)
        template<class T, class PM>
        using pointer_to_member =
            std::enable_if_t<
                std::is_member_pointer_v<PM> &&
    ...
    template<class T>
    inline constexpr bool has_arrow = std::is_pointer_v<std::remove_reference_t<T>> || ::snap::is_detected_v<op::member_arrow, T>;
} // namespace snap
