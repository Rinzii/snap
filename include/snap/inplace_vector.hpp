...
struct inplace_vector {
    ...
    inplace_vector(size_type count, const T& value) {
        ...
        try {
            ...
            while (i > 0) { --i; (base()[i]).~T(); }
            throw;
        }
    }

	// (4) Constructs an inplace_vector with the contents of the range [first, last).
    template<class InputIt, class = std::enable_if_t<!std::is_integral<InputIt>::value>>
    ...
    template<class R,
             class B = decltype(std::begin(std::declval<R&>())),
             class E = decltype(std::end  (std::declval<R&>()))>
    explicit inplace_vector(R&& rg) {
    ...
    constexpr inplace_vector(std::initializer_list<T> il) {
        ...
        size_type i = 0;
        try {
            for (const T& v : il) { ::new (static_cast<void*>(base() + i)) T(v); ++i; }
            m_size = il.size();
        } catch (...) {
            while (i > 0) { --i; (base()[i]).~T(); }
            throw;
    ...
    inplace_vector(inplace_vector&& other) noexcept(std::is_nothrow_move_constructible<T>::value) {
        ...
        if constexpr (std::is_nothrow_move_constructible<T>::value) {
            for (size_type i = 0; i < other.m_size; ++i)
                ::new (static_cast<void*>(base() + i)) T(std::move(other.base()[i]));
            m_size = other.m_size;
            other.destroy_all_();
            other.m_size = 0;
        } else {
    ...
    inplace_vector& operator=(inplace_vector&& rhs) noexcept(std::is_nothrow_move_constructible<T>::value) {
        ...
        if constexpr (std::is_nothrow_move_constructible<T>::value) {
            ...
            for (size_type i = 0; i < rhs.m_size; ++i)
                ::new (static_cast<void*>(base() + i)) T(std::move(rhs.base()[i]));
            m_size = rhs.m_size;
            rhs.destroy_all_();
            rhs.m_size = 0;
        } else {
    ...
    void assign(InputIt first, InputIt last) {
        ...
        try {
            for (; first != last; ++first) {
                if (collected == N) { // more elements exist -> capacity exceeded
                    while (collected > 0) { --collected; buf[collected].~T(); }
                    throw std::bad_alloc();
                }
            ...
            // commit
            clear();
            for (size_type i = 0; i < collected; ++i)
                ::new (static_cast<void*>(base() + i)) T(std::move(buf[i]));
            m_size = collected;
            while (collected > 0) { --collected; buf[collected].~T(); }
        } catch (...) {
            while (collected > 0) { --collected; buf[collected].~T(); }
    ...
    iterator insert(const_iterator pos, size_type count, const T& value) {
        ...
        size_type moved = 0;
        try {
            for (; moved < tail; ++moved) {
                ::new (static_cast<void*>(base() + m_size + count - 1 - moved))
                    T(std::move(base()[m_size - 1 - moved]));
            }
        } catch (...) {
            while (moved > 0) { --moved; (base()[m_size + count - 1 - moved]).~T(); }
            throw;
        }
        ...
        try {
            for (; built < count; ++built)
                ::new (static_cast<void*>(base() + idx + built)) T(value);
        } catch (...) {
            while (built > 0) { --built; (base()[idx + built]).~T(); }
            while (moved > 0) { --moved; (base()[m_size + count - 1 - moved]).~T(); }
            throw;
        }
    ...
    iterator insert(const_iterator pos, InputIt first, InputIt last) {
        ...
        if (std::is_base_of<std::forward_iterator_tag, cat>::value) {
            ...
            try {
                ...
                while (built > 0) { --built; (tmp[built]).~T(); }
                throw;
            }
        }
    ...
    iterator emplace(const_iterator pos, Args&&... args) {
        ...
        try {
            for (; moved < tail; ++moved) {
                ::new (static_cast<void*>(base() + m_size      - moved))
                    T(std::move(base()[m_size - 1 - moved]));
            }
        } catch (...) {
            while (moved > 0) { --moved; (base()[m_size - moved]).~T(); }
            throw;
        }
    ...
    void append_range(InputIt first, InputIt last) {
        ...
        try {
            for (; first != last; ++first) {
                if (built == avail) { // still more elements → capacity exceeded
                    while (built > 0) { --built; (buf[built]).~T(); }
                    throw std::bad_alloc();
                }
            ...
            for (size_type i = 0; i < built; ++i)
                unchecked_emplace_back(std::move(buf[i]));
            while (built > 0) { --built; (buf[built]).~T(); }
        } catch (...) {
            while (built > 0) { --built; (buf[built]).~T(); }
            throw;
    ...
    template<class R,
             class B = decltype(std::begin(std::declval<R&>())),
             class E = decltype(std::end  (std::declval<R&>()))>
    void append_range(R&& rg) { append_range(std::begin(rg), std::end(rg)); }

    // try_append_range: returns iterator to first not-inserted (or end of range)
    template<class InputIt>
    InputIt try_append_range(InputIt first, InputIt last) {
    ...
    template<class R,
             class B = decltype(std::begin(std::declval<R&>())),
             class E = decltype(std::end  (std::declval<R&>()))>
    auto try_append_range(R&& rg) -> decltype(std::begin(rg)) {
        auto it  = std::begin(rg);
        auto end = std::end(rg);
        for (; it != end && m_size < N; ++it)
                                              ...
                                              noexcept(std::swap(std::declval<T&>(), std::declval<T&>())))
    {
        ...
        if (m_size > other.m_size) {
            ...
            try {
                for (; j < extra; ++j)
                    ::new (static_cast<void*>(other.base() + other.m_size + j))
                        T(std::move(base()[m + j]));
                ...
                while (j > 0) { --j; (other.base()[other.m_size + j]).~T(); }
                throw;
            }
            ...
            try {
                for (; j < extra; ++j)
                    ::new (static_cast<void*>(base() + m_size + j))
                        T(std::move(other.base()[m + j]));
                for (size_type k = 0; k < extra; ++k) (other.base()[m + k]).~T();
                m_size += extra;
                other.m_size -= extra;
            } catch (...) {
                while (j > 0) { --j; (base()[m_size + j]).~T(); }
                throw;
            }
        }
    }
...
struct inplace_vector<T, 0> {
    ...
    explicit inplace_vector(size_type count)              { if (count) throw std::bad_alloc(); }
    inplace_vector(size_type, const T&)                   { throw std::bad_alloc(); }
    template<class It, class = std::enable_if_t<!std::is_integral<It>::value>>
    inplace_vector(It first, It last)                     { if (first != last) throw std::bad_alloc(); }
    template<class R, class B = decltype(std::begin(std::declval<R&>())),
                     class E = decltype(std::end  (std::declval<R&>()))>
    explicit inplace_vector(R&& rg)                       { if (std::begin(rg) != std::end(rg)) throw std::bad_alloc(); }
    inplace_vector(std::initializer_list<T> il)           { if (il.size()) throw std::bad_alloc(); }

    // assignment / modifiers consistent with capacity=0
    inplace_vector& operator=(std::initializer_list<T> il){ if (il.size()) throw std::bad_alloc(); return *this; }

    void assign(size_type count, const T&)                { if (count) throw std::bad_alloc(); }
    template<class It, class = std::enable_if_t<!std::is_integral<It>::value>>
    void assign(It first, It last)                        { if (first != last) throw std::bad_alloc(); }
    template<class R, class B = decltype(std::begin(std::declval<R&>())),
                     class E = decltype(std::end  (std::declval<R&>()))>
    void assign_range(R&& rg)                             { if (std::begin(rg) != std::end(rg)) throw std::bad_alloc(); }
    ...
    void resize(size_type count)                   { if (count) throw std::bad_alloc(); }
    void resize(size_type count, const T&)         { if (count) throw std::bad_alloc(); }

    template<class It> void append_range(It first, It last) { if (first != last) throw std::bad_alloc(); }
    template<class R, class B = decltype(std::begin(std::declval<R&>())),
                     class E = decltype(std::end  (std::declval<R&>()))>
    void append_range(R&& rg) { if (std::begin(rg) != std::end(rg)) throw std::bad_alloc(); }

    template<class It> It try_append_range(It first, It last) { return first; }
    template<class R, class B = decltype(std::begin(std::declval<R&>())),
                     class E = decltype(std::end  (std::declval<R&>()))>
    auto try_append_range(R&& rg) -> decltype(std::begin(rg)) { return std::begin(rg); }

    template<class... Args> iterator emplace(const_iterator pos, Args&&...) { (void)pos; throw std::bad_alloc(); }

    iterator insert(const_iterator pos, const T&) { (void)pos; throw std::bad_alloc(); }
    iterator insert(const_iterator pos, T&&)      { (void)pos; throw std::bad_alloc(); }
    iterator insert(const_iterator pos, size_type count, const T&) { (void)pos; if (count) throw std::bad_alloc(); return nullptr; }

    template<class It, class = std::enable_if_t<!std::is_integral<It>::value>>
    iterator insert(const_iterator pos, It first, It last) { (void)pos; if (first != last) throw std::bad_alloc(); return nullptr; }

    template<class R, class B = decltype(std::begin(std::declval<R&>())),
                     class E = decltype(std::end  (std::declval<R&>()))>
    iterator insert_range(const_iterator pos, R&& rg) { (void)pos; if (std::begin(rg) != std::end(rg)) throw std::bad_alloc(); return nullptr; }

    iterator erase(const_iterator pos) { (void)pos; return nullptr; }
...
erase(inplace_vector<T, N>& c, const U& value) {
    ...
    for (auto it = c.begin(); it != c.end(); /* no inc */) {
        if (*it == value) { it = c.erase(it); ++removed; }
        else { ++it; }
    }
    return removed;
}
...
erase_if(inplace_vector<T, N>& c, Pred pred) {
    ...
    size_type removed = 0;
    for (auto it = c.begin(); it != c.end(); /* no inc */) {
        if (pred(*it)) { it = c.erase(it); ++removed; }
        else { ++it; }
    }
    return removed;
}
...
// main.cpp (C++17)
// Build: g++ -std=gnu++17 -O2 -Wall -Wextra main.cpp
...
static void expect_eq(const C& c, std::initializer_list<T> il) {
    assert(static_cast<std::size_t>(std::distance(c.begin(), c.end())) == il.size());
    assert(std::equal(c.begin(), c.end(), il.begin(), il.end()));
}

// A minimal single-pass input iterator to exercise the range ctor
template <class T>
struct input_it {
    ...
    input_it() = default;
    explicit input_it(pointer x) : p(x) {}

    reference operator*()  const { return *p; }
    pointer   operator->() const { return  p; }

    input_it& operator++()       { ++p; return *this; }
    input_it  operator++(int)    { input_it tmp{*this}; ++(*this); return tmp; }

    bool operator==(const input_it& rhs) const { return p == rhs.p; }
    bool operator!=(const input_it& rhs) const { return p != rhs.p; }
};

// Type that tracks alive objects and can throw on copy/move to test unwinding
struct Tracked {
    static int alive;
    static int copies_left; // if this hits < 0 on a copy, throw
    static int moves_left;  // if this hits < 0 on a move, throw
    ...
    Tracked(const Tracked& other) : v(other.v) {
        if (--copies_left < 0) throw std::runtime_error("copy throw");
        ++alive;
    }

    Tracked(Tracked&& other) noexcept(false) : v(other.v) {
        if (--moves_left < 0) throw std::runtime_error("move throw");
        ++alive;
    }

    Tracked& operator=(const Tracked& other) {
        if (--copies_left < 0) throw std::runtime_error("copy assign throw");
        v = other.v;
        return *this;
    }

    Tracked& operator=(Tracked&& other) noexcept(false) {
        if (--moves_left < 0) throw std::runtime_error("move assign throw");
        v = other.v;
        return *this;
    }
    ...
    static void reset_budget(int copy_budget, int move_budget) {
        copies_left = copy_budget;
        moves_left  = move_budget;
    }
};
int Tracked::alive = 0;
int Tracked::copies_left = 1000000;
int Tracked::moves_left  = 1000000;
...
int main() {
    ...
    {
        using I = snap::inplace_vector<int, 4>;
        static_assert(std::is_same<typename I::value_type, int>::value, "");
        static_assert(std::is_same<typename I::size_type, std::size_t>::value, "");
        static_assert(std::is_same<typename I::difference_type, std::ptrdiff_t>::value, "");
        static_assert(std::is_same<typename I::reference, int&>::value, "");
        static_assert(std::is_same<typename I::const_reference, const int&>::value, "");
        static_assert(std::is_same<typename I::pointer, int*>::value, "");
        static_assert(std::is_same<typename I::const_pointer, const int*>::value, "");
        static_assert(std::is_same<typename I::iterator, int*>::value, "");
        static_assert(std::is_same<typename I::const_iterator, const int*>::value, "");
        static_assert(I::static_capacity == 4, "");
        assert(I::capacity() == 4);
    }

    // default ctor, data/iterators on empty, contiguity after fill
    {
        using I = snap::inplace_vector<int, 5>;
        I a;
        assert(a.size() == 0);
        assert(a.empty());
        ...
        I b(3); // default-inserted
        assert(b.size() == 3);
        int sum = 0;
        for (int x : b) sum += x;
        assert(sum == 0);
        assert(b.data() == &*b.begin());
        assert(&b.begin()[2] == b.data() + 2);

        I c(4, 7); // value fill
        expect_eq(c, {7,7,7,7});
        assert(c.data() && &*(c.end()-1) == c.data() + (c.size()-1));
    }

    // range constructor: pointers and single-pass input iterator
    {
        using I = snap::inplace_vector<int, 6>;
        int raw[] = {1,2,3,4};
        I pctor(raw, raw+4);
        expect_eq(pctor, {1,2,3,4});

        input_it<int> f(raw), l(raw+3);
        I ictr(f, l);
        expect_eq(ictr, {1,2,3});
    }

    // initializer_list, copy, move
    {
        using I = snap::inplace_vector<int, 8>;
        I a{9,8,7,6};
        expect_eq(a, {9,8,7,6});
        ...
        I c = std::move(a); // move
        expect_eq(c, {9,8,7,6});
        assert(a.size() == 0); // moved-from becomes empty in this impl
    }

    // reverse iterators and const iterators
    {
        using I = snap::inplace_vector<int, 5>;
        I v{1,2,3};
        const I& cv = v;
        assert(std::distance(v.begin(), v.end()) == 3);
        assert(std::distance(cv.begin(), cv.end()) == 3);
        assert(*v.rbegin() == 3);
        assert(*std::prev(v.rend()) == 1);
        assert(&*cv.begin() == v.data());
    }

    // capacity errors on construction (count / ilist / range)
    {
        using I2 = snap::inplace_vector<int, 2>;

        bool threw = false;
        try { I2 a(3); } catch (const std::length_error&) { threw = true; }
        assert(threw);

        threw = false;
        try { I2 b{1,2,3}; } catch (const std::length_error&) { threw = true; }
        assert(threw);

        int arr[] = {1,2,3};
        threw = false;
        try { I2 c(arr, arr+3); } catch (const std::length_error&) { threw = true; }
        assert(threw);
    }

    // exception unwinding: range ctor (copy throws part-way)
    {
        Tracked::alive = 0;
        Tracked::reset_budget(/*copy_budget*/1, /*move_budget*/1000000);

        Tracked src[3] = {Tracked(1), Tracked(2), Tracked(3)}; // alive += 3
        assert(Tracked::alive == 3);

        bool threw = false;
        try {
            // copy budget allows 1 successful copy, then throws during 2nd or 3rd
            snap::inplace_vector<Tracked, 3> bad(src, src+3);
            (void)bad;
            assert(false && "should not reach");
        } catch (const std::exception&) { threw = true; }
        assert(threw);

        // All partially-constructed elements must have been destroyed
        assert(Tracked::alive == 3);

        // Clean up src to get back to zero
        (void)src;
        // leaving scope will destruct src[3], alive -> 0
    }
    assert(Tracked::alive == 0);

    // exception unwinding: count+value ctor (copy throws part-way)
    {
        Tracked::alive = 0;
        Tracked value(42);                  // alive = 1
        Tracked::reset_budget(/*copy_budget*/1, /*move_budget*/1000000);

        bool threw = false;
        try {
            snap::inplace_vector<Tracked, 4> bad(3, value);
            (void)bad;
            assert(false && "should not reach");
        } catch (const std::exception&) { threw = true; }
        assert(threw);

        // Only 'value' remains alive
        assert(Tracked::alive == 1);
    }
    assert(Tracked::alive == 0); // value destroyed after block

    // data() non-null only when non-empty, contiguity checks across types
    {
        snap::inplace_vector<char, 5> a;
        assert(a.data() == nullptr);
        snap::inplace_vector<char, 5> b(2, 'x');
        assert(b.data() != nullptr);
        assert(*(b.data()+0) == 'x' && *(b.data()+1) == 'x');
        assert(&*b.begin() == b.data());
        assert(&*(b.end()-1) == b.data() + (b.size()-1));
    }
