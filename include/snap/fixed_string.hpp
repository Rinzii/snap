...
// For feature-test macros like __cpp_lib_containers_ranges
...
// TODO: Should I move this into a separate header file?
#if defined(__cpp_lib_containers_ranges) && (__cpp_lib_containers_ranges >= 202202L)
  using from_range_t = std::from_range_t;
  inline constexpr from_range_t from_range{};
#else
  struct from_range_t { constexpr from_range_t() = default; };
  inline constexpr from_range_t from_range{};
#endif
...
namespace details {
    ...
    struct is_neq_comparable<T, U, std::void_t<
        decltype(std::declval<const T&>() != std::declval<const U&>())
    >> : std::true_type {};
    template<class T, class U>
    inline constexpr bool is_neq_comparable_v = is_neq_comparable<T, U>::value;
...
{}
  ...
  SNAP_CONSTEVAL SNAP_EXPLICIT_EXPR_ONLY(false) basic_fixed_string(const CharT (&txt)[N + 1]) noexcept
  {
    ...
    constexpr explicit basic_fixed_string(It begin, S end)
    {
        ...
        static_assert(std::is_base_of_v<std::input_iterator_tag, cat>,
                    "It must model an input iterator (C++17 tag check)");
        static_assert(std::is_same_v<std::remove_cv_t<val>, CharT>,
                    "iterator value_type must be CharT");
        static_assert(details::is_neq_comparable_v<It, S>,
                    "begin and end must be comparable with !=");
...
constexpr basic_fixed_string(std::from_range_t, R&& r)
{
    ...
    for (auto it = m_data; auto&& v : std::forward<R>(r))
    {
...
// capacity
                                                                            ...
                                                                            const basic_fixed_string& rhs) noexcept
  {
   ...
   template<std::size_t N2>
  [[nodiscard]] SNAP_CONSTEVAL friend basic_fixed_string<CharT, N + N2 - 1> operator+(const basic_fixed_string& lhs,
  ...
  [[nodiscard]] SNAP_CONSTEVAL friend basic_fixed_string<CharT, N1 + N - 1> operator+(const CharT (&lhs)[N1],
                                                                                 const basic_fixed_string& rhs) noexcept
  {
                                                 ...
                                                 const basic_fixed_string<CharT, N2>& rhs)
  {
    ...
    template<std::size_t N2>
    [[nodiscard]] friend SNAP_CONSTEVAL bool operator==(const basic_fixed_string& lhs, const CharT (&rhs)[N2]) noexcept
    {
  ...
  template<std::size_t N2>
  [[nodiscard]] friend SNAP_CONSTEVAL auto operator<=>(const basic_fixed_string& lhs, const CharT (&rhs)[N2])
  {
    assert(rhs[N2 - 1] == CharT{});
...
template<class CharT, class... Rest,
         std::enable_if_t<
             std::conjunction_v<std::is_same<Rest, CharT>...>, int> = 0>
...
template<class CharT, std::size_t N>
constexpr void swap(basic_fixed_string<CharT, N>& x,
...
template<std::size_t N>
struct std::hash<snap::fixed_string<N>> : std::hash<std::string_view> {};

#if defined(__cpp_char8_t)
#  include <string_view> // for std::u8string_view
template<std::size_t N>
struct std::hash<snap::fixed_u8string<N>> : std::hash<std::u8string_view> {};
#endif

template<std::size_t N>
struct std::hash<snap::fixed_u16string<N>> : std::hash<std::u16string_view> {};
template<std::size_t N>
struct std::hash<snap::fixed_u32string<N>> : std::hash<std::u32string_view> {};
template<std::size_t N>
struct std::hash<snap::fixed_wstring<N>>  : std::hash<std::wstring_view> {};
...
namespace std {
    template<class CharT, std::size_t N, class ParseCharT>
    struct formatter<snap::basic_fixed_string<CharT, N>, ParseCharT>
        : formatter<std::basic_string_view<CharT>, ParseCharT>
    {
        template<class FormatContext>
        auto format(const snap::basic_fixed_string<CharT, N>& str,
