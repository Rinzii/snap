#ifndef SNP_INCLUDE_SNAP_INTERNAL_HELPERS_PERFECT_FORWARD_HPP
#define SNP_INCLUDE_SNAP_INTERNAL_HELPERS_PERFECT_FORWARD_HPP
...
namespace internal
{
	...
	// Forward declaration
	template <class Op, class Indices, class... BoundArgs> struct perfect_forward_impl;

	// Implementation over an index sequence
	template <class Op, std::size_t... I, class... BoundArgs> struct perfect_forward_impl<Op, std::index_sequence<I...>, BoundArgs...>
	{
	private:
	...
	public:
		...
		explicit constexpr perfect_forward_impl(Args&&... bound_args) noexcept(std::is_nothrow_constructible_v<std::tuple<BoundArgs...>, Args&&...>)
		...
		constexpr auto operator()(Args&&... args) & noexcept(noexcept(Op{}(std::get<I>(bound_args_)..., std::forward<Args>(args)...)))
		...
		constexpr auto operator()(Args&&... args) const& noexcept(noexcept(Op{}(std::get<I>(bound_args_)..., std::forward<Args>(args)...)))
			-> decltype(Op{}(std::get<I>(bound_args_)..., std::forward<Args>(args)...))
		{
			return Op{}(std::get<I>(bound_args_)..., std::forward<Args>(args)...);
		}
		...
		// && overload
		template <class... Args, class = std::enable_if_t<std::is_invocable_v<Op, BoundArgs..., Args...>>>
		constexpr auto operator()(Args&&... args) && noexcept(noexcept(Op{}(std::get<I>(std::move(bound_args_))..., std::forward<Args>(args)...)))
		...
		// const&& overload
		template <class... Args, class = std::enable_if_t<std::is_invocable_v<Op, const BoundArgs..., Args...>>>
		constexpr auto operator()(Args&&... args) const&& noexcept(noexcept(Op{}(std::get<I>(std::move(bound_args_))..., std::forward<Args>(args)...)))
			-> decltype(Op{}(std::get<I>(std::move(bound_args_))..., std::forward<Args>(args)...))
		{
			return Op{}(std::get<I>(std::move(bound_args_))..., std::forward<Args>(args)...);
		}
...
#endif // SNP_INCLUDE_SNAP_INTERNAL_HELPERS_PERFECT_FORWARD_HPP
