...
#  if defined(__GNUC__) && (__GNUC__ > 11 || (__GNUC__ == 11 && __GNUC_MINOR__ >= 1)) \
      && !defined(__clang__) && !defined(__NVCOMPILER) && !defined(__NVCOMPILER_LLVM__) && !defined(__CUDACC__)
...
namespace snap::builtin {
    ...
    template <class>
    inline constexpr bool has_constexpr_bit_cast_dep = has_constexpr_bit_cast;

    template <typename To, typename From>
    constexpr std::enable_if_t<
        has_constexpr_bit_cast_dep<To> &&
