...
template <class T>
struct is_forbidden_char_base : std::false_type {};
...
template <class T>
inline constexpr bool is_forbidden_char_v =
    is_forbidden_char_base<remove_cv_t<T>>::value;

template <class T>
inline constexpr bool is_allowed_cmp_int_v =
    std::is_integral_v<T> &&
    !std::is_same_v<remove_cv_t<T>, bool> &&
    !is_forbidden_char_v<T>;

template <class T, class U>
using enable_cmp_t = std::enable_if_t<is_allowed_cmp_int_v<T> && is_allowed_cmp_int_v<U>, int>;

} // namespace detail

// cmp_equal
template <class T, class U, detail::enable_cmp_t<T, U> = 0>
