...
using copy_cvref_t =
    std::conditional_t<
        std::is_lvalue_reference_v<From>, std::add_lvalue_reference_t<copy_cv_t<From, To>>,
        std::conditional_t<
            std::is_rvalue_reference_v<From>, std::add_rvalue_reference_t<copy_cv_t<From, To>>,
            copy_cv_t<From, To>
        >
    >;
...
template <class A, class B, class X = std::remove_reference_t<A>, class Y = std::remove_reference_t<B>>
struct common_ref_impl {}; // fallback
...
template <typename T>
struct common_ref_impl<T&&, T&, T, T> { using type = T&; };
template <typename T>
struct common_ref_impl<T&, T&&, T, T> { using type = T&; };
...
public:
    using type = std::enable_if_t<
        std::is_convertible_v<A&&, C> && std::is_convertible_v<B&&, C>,
        C
    >;
};
...
private:
    ...
    template <class X = T, class Y = U,
              class Ref = typename internal::common_ref_impl<X, Y>::type>
    static Ref test(priority_tag<4>);

    // 2. basic_common_reference is well-formed
    template <class X = T, class Y = U,
              class Basic = typename basic_common_reference<
                  snap::remove_cvref_t<X>,
                  snap::remove_cvref_t<Y>,
                  internal::xref<X>::template apply,
                  internal::xref<Y>::template apply
              >::type>
    static Basic test(priority_tag<3>);

    // 3. conditional result
    template <class X = T, class Y = U,
              class Cond = internal::cond_res_t<X, Y>>
    static Cond test(priority_tag<2>);

    // 4. fallback to common_type
    template <class X = T, class Y = U,
              class CT = std::common_type_t<X, Y>>
    static CT test(priority_tag<1>);
...
template<class... Types>
using is_common_reference_detected = internal::has_type_member<common_reference<Types...>>;
