...
class atomic_unique_lock {
  static_assert(LockedBit != 0 && (LockedBit & (LockedBit - 1)) == 0,
                "LockedBit must have exactly one bit set");

public:
  explicit atomic_unique_lock(std::atomic<State>& state) noexcept
      : state_(state), is_locked_(false) {
    lock();
  }

  template <class Pred>
  atomic_unique_lock(std::atomic<State>& state, Pred&& give_up_locking) noexcept
      : state_(state), is_locked_(false) {
    is_locked_ = lock_impl(std::forward<Pred>(give_up_locking),
                           set_locked_bit,
                           std::memory_order_acquire);
  }

  template <class Pred, class UnaryFunction>
  atomic_unique_lock(std::atomic<State>& state,
                     Pred&& give_up_locking,
                     UnaryFunction&& state_after_lock,
                     std::memory_order locked_ordering) noexcept
      : state_(state), is_locked_(false) {
    is_locked_ = lock_impl(std::forward<Pred>(give_up_locking),
                           std::forward<UnaryFunction>(state_after_lock),
                           locked_ordering);
  }
...
private:
  template <class Pred, class UnaryFunction>
  bool lock_impl(Pred&& give_up_locking,
                 UnaryFunction&& state_after_lock,
                 std::memory_order locked_ordering) noexcept {
