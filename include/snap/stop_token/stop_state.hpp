...
public:
  ...
  void increment_stop_source_counter() noexcept {
    ...
    (void)cur;
    assert((cur >> stop_source_counter_shift) !=
           (std::numeric_limits<state_t>::max() >> stop_source_counter_shift) &&
           "stop_source counter overflow");
  ...
  bool stop_requested() const noexcept {
    return (state_.load(std::memory_order_acquire) & stop_requested_bit) != 0;
  }
  ...
  void remove_callback(stop_callback_base* cb) noexcept {
    ...
    const bool potentially_executing_now =
        cb->prev == nullptr && !callbacks_.is_head(cb);
...
private:
  list_lock try_lock_for_request_stop() noexcept {
    const auto lock_fail = [](state_t s) {
      return (s & stop_requested_bit) != 0;
    };
    const auto after_lock = [](state_t s) {
      return static_cast<state_t>(s | callback_list_locked_bit | stop_requested_bit);
    };
  ...
  template <class T>
  friend struct intrusive_shared_ptr_traits;
};

// intrusive_shared_ptr trait specialization for stop_state.
template <>
struct intrusive_shared_ptr_traits<stop_state> {
  static std::atomic<std::uint32_t>& get_atomic_ref_count(stop_state& s) noexcept {
    return s.ref_count_;
  }
};
