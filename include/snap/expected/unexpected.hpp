...
namespace snap
{
	...
	template <class T> inline constexpr bool is_unexpected_v = is_unexpected<T>::value;
	...
	template <class E> class unexpected
	{
		...
		static_assert(!std::is_const_v<E> && !std::is_volatile_v<E>, "unexpected<E>: E must not be cv-qualified");
		static_assert(!is_unexpected_v<E>, "unexpected<E>: E must not be a specialization of unexpected");
	...
	public:
		...
		template <class Err,
				  class = std::enable_if_t<!std::is_same_v<remove_cvref_t<Err>, unexpected> && !std::is_same_v<remove_cvref_t<Err>, std::in_place_t> &&
										   std::is_constructible_v<E, Err&&>>>
		explicit constexpr unexpected(Err&& e) noexcept(std::is_nothrow_constructible_v<E, Err&&>) : err_(std::forward<Err>(e))
		{
		}

		// 4) in_place + args... (brace-init so aggregates work)
		template <class... Args, class = std::enable_if_t<is_brace_constructible_v<E, Args&&...>>>
		explicit constexpr unexpected(std::in_place_t, Args&&... args) noexcept(noexcept(E{ std::forward<Args>(args)... }))
		...
		explicit constexpr unexpected(std::in_place_t, std::initializer_list<U> il, Args&&... args) noexcept(noexcept(E{ il, std::forward<Args>(args)... }))
		...
		template <class E2, class = std::enable_if_t<std::is_convertible_v<decltype(std::declval<const E&>() == std::declval<const E2&>()), bool>>>
