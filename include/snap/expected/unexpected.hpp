...
public:
	...
	template <class X = E, class = std::enable_if_t<std::is_swappable_v<X>>> constexpr void swap(unexpected& other) noexcept(std::is_nothrow_swappable_v<X>)
	{
		using std::swap;
		swap(err_, other.err_);
	}

	template <class E2> friend constexpr bool operator==(const unexpected& x, const unexpected<E2>& y) { return x.error() == y.error(); }
