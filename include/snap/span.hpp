...
namespace internal_span_detail
{
	...
	namespace detail
	{
		...
		constexpr std::size_t size(const std::array<T, N>& arr) noexcept
		{
			return arr.size();
		}

		template <class T, std::size_t N>
		constexpr std::size_t size(const T (&array)[N]) noexcept // NOLINT(cppcoreguidelines-avoid-c-arrays)
		{
			return N;
		}
		...
		template <class T, std::size_t N>
		constexpr T* data(std::array<T, N>& arr) noexcept
		{
			return arr.data();
		}

		template <class T, std::size_t N>
		constexpr const T* data(const std::array<T, N>& arr) noexcept
		{
			return arr.data();
		}

		template <class T, std::size_t N>
		constexpr T* data(T (&array)[N]) noexcept // NOLINT(cppcoreguidelines-avoid-c-arrays)
		{
			return array;
		}
		...
		struct is_container
		{
			static constexpr bool value =
				!::SNAP_NAMESPACE::is_span<U>::value && !is_std_array<U>::value && !std::is_array_v<U> && has_size_and_data<C>::value;
		};
		...
		struct is_container_element_type_compatible<
			...
			std::enable_if_t<!std::is_same_v<std::remove_cv_t<decltype(detail::data(std::declval<T>()))>, void> &&
							 std::is_convertible_v<std::remove_pointer_t<decltype(detail::data(std::declval<T>()))> (*)[], E (*)[]>> // NOLINT(cppcoreguidelines-avoid-c-arrays)
		...
		template <typename E>
		struct span_storage<E, static_cast<std::size_t>(-1)>
		{
			constexpr span_storage() noexcept = default;
...
#		define SNAP_SPAN_EXPECT(cond)                                                                                                                            \
			((cond) ? static_cast<void>(0)                                                                                                                      \
					: ::SNAP_NAMESPACE::internal_span_detail::contract_violation("Expected " SNAP_SPAN_STRINGIFY(cond)))
...
public:
	...
	constexpr span() noexcept : storage_(nullptr, size_type{})
	{
	}
	...
	{
		SNAP_SPAN_EXPECT(extent == dynamic_extent || count == extent);
	}

	/// Constructs a span over `[first_elem, last_elem)`.
	constexpr span(pointer first_elem, pointer last_elem)
		: storage_(first_elem, static_cast<size_type>(last_elem - first_elem))
	{
	...
	template <std::size_t N,
			  ...
			  std::enable_if_t<
				  (E == dynamic_extent || N == E) &&
					  internal_span_detail::detail::is_container_element_type_compatible<element_type (&)[N], ElementType>::value, // NOLINT(cppcoreguidelines-avoid-c-arrays)
				  int> = 0>
	...
	template <typename Container,
			  std::size_t E = Extent,
			  std::enable_if_t<
				  E == dynamic_extent && internal_span_detail::detail::is_container<Container>::value &&
	...
	using subspan_return_t = span<ElementType, subspan_extent<Offset, Count>::value>;
...
template <class T, std::size_t N>
span(T (&)[N]) -> span<T, N>; // NOLINT(cppcoreguidelines-avoid-c-arrays)
...
span(Container&) -> span<std::remove_reference_t<decltype(*internal_span_detail::detail::data(std::declval<Container&>()))>, dynamic_extent>;

template <class Container>
span(const Container&) -> span<const typename Container::value_type, dynamic_extent>;
...
template <typename T, std::size_t N>
constexpr span<T, N> make_span(T (&arr)[N]) noexcept // NOLINT(cppcoreguidelines-avoid-c-arrays)
{
	return {arr};
}
...
template <typename Container>
constexpr span<std::remove_reference_t<decltype(*internal_span_detail::detail::data(std::declval<Container&>()))>, dynamic_extent>
...
template <typename Container>
constexpr span<const typename Container::value_type, dynamic_extent> make_span(const Container& cont)
{
	return {cont};
}
...
constexpr decltype(auto) get(span<E, S> s)
{
	return s[N];
}
...
{
	template <typename ElementType, std::size_t Extent>
	class tuple_size<::SNAP_NAMESPACE::span<ElementType, Extent>> : public integral_constant<std::size_t, Extent>
	{
	};

	template <typename ElementType>
	class tuple_size<::SNAP_NAMESPACE::span<ElementType, ::SNAP_NAMESPACE::dynamic_extent>>;

	template <std::size_t I, typename ElementType, std::size_t Extent>
	class tuple_element<I, ::SNAP_NAMESPACE::span<ElementType, Extent>>
	{
	public:
