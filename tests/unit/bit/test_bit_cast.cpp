...
// byte helpers
template <class T> static std::array<unsigned char, sizeof(T)> to_bytes(const T& v)
{
...
template <class T> static T from_bytes(std::array<unsigned char, sizeof(T)> b)
{
	T v{};
...
template <class T, std::size_t N> constexpr bool arr_eq(const std::array<T, N>& a, const std::array<T, N>& b)
{
...
// samples
template <class T, class Enable = void> struct Samples;

template <class T> struct Samples<T, std::enable_if_t<std::is_integral_v<T>>>
{
	static std::vector<T> values()
	{
		using U = std::make_unsigned_t<T>;
		return {
			static_cast<T>(0),			static_cast<T>(1),			static_cast<T>(~U{ 0 }),
			static_cast<T>(U{ 0x55U }), static_cast<T>(U{ 0xAAU }), static_cast<T>(U{ 0x12345678U & ~U{ 0 } }) // masked to width
		};
	}
};

template <class T> struct Samples<T, std::enable_if_t<std::is_floating_point_v<T>>>
{
	static std::vector<T> values()
	{
		...
		if (std::numeric_limits<T>::has_quiet_NaN) { v.push_back(std::numeric_limits<T>::quiet_NaN()); }
		if (std::numeric_limits<T>::denorm_min() > T(0)) { v.push_back(std::numeric_limits<T>::denorm_min()); }
		return v;
	}
};

template <size_t N> static std::vector<std::array<unsigned char, N>> array_patterns()
{
	std::array<unsigned char, N> const zeros{};
...
template <class T> struct Samples<T, std::enable_if_t<is_uc_array_v<T>>>
{
	static std::vector<T> values()
	{
...
template <> struct Samples<TwoU16, void>
{
	static std::vector<TwoU16> values() { return { TwoU16{ 0, 0 }, TwoU16{ 1, 0xFFFFU }, TwoU16{ 0xA55AU, 0x5AA5U }, TwoU16{ 0x1234U, 0x5678U } }; }
};

// typed pairs
template <class To_, class From_> struct Pair
{
	using To   = To_;
	using From = From_;
};

using BitCastPairs = ::testing::Types<Pair<std::uint32_t, float>,
...
template <class P> class BitCastTyped : public ::testing::Test
{
protected:
