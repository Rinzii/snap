...
template <class T>
constexpr int reference_countl_zero(T value)
{
...
template <class T>
constexpr int reference_countl_one(T value)
{
...
template <class T>
constexpr int reference_countr_zero(T value)
{
  if (value == T{0}) return digits_v<T>;
  int count = 0;
...
template <class T>
constexpr int reference_countr_one(T value)
{
  if (value == std::numeric_limits<T>::max()) return digits_v<T>;
  int count = 0;
  for (int bit = 0; bit < digits_v<T>; ++bit) {
...
template <class T>
class CountLeadingTrailingTyped : public ::testing::Test { };
